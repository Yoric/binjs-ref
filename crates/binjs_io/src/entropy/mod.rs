...
use self::dictionary::{Dictionary, DictionaryFamily, UserExtensibleTables};
...
pub struct Options {
    ...
    probabilities: DictionaryFamily<SymbolInfo>,

    known_values: UserExtensibleTables,
...
impl Options {
    ...
    ///
    /// If `probabilities` is empty, a default dictionary is insertred.
    pub fn new(
        spec: &binjs_meta::spec::Spec,
        mut probabilities: DictionaryFamily<Instances>,
        known_values: UserExtensibleTables,
    ) -> Self {
        ...
        for dictionary in probabilities.values_mut() {
            *dictionary = dictionary.with_grammar_fallback(baseline.clone());
        }
        assert!(
            !probabilities.insert_baseline(baseline),
            "The dictionary family MUST start without a definition for dictionary `*`"
        );
        let mut probabilities = probabilities.instances_to_probabilities("dictionary");
        probabilities
    ...
    pub fn dictionary_options(&self) -> dictionary::Options {
        dictionary::Options::default()
            ...
            .with_width(
                1, /* FIXME: Placeholder - we should remove all this window stuff */
            )
    }
    ...
    fn subcommand<'a, 'b>(&self) -> clap::App<'a, 'b> {
        ...
        SubCommand::with_name("entropy")
            ...
            .arg(Arg::with_name("cdf")
                .help("(Recommended) path to external prediction tables generated by binjs_generate_prediction_tables.")
                .long("cdf")
                .takes_value(true)
                .required(false)
            )
            .arg(Arg::with_name("values")
                .help("(Recommended) path to a dictionary of well-known values generated by binjs_generate_prediction_tables.")
                .long("values")
                .takes_value(true)
                .required(false)
            )
    ...
    fn handle_subcommand(
        ...
        let probabilities = match matches.value_of("cdf") {
            ...
            Some(path) => {
                ...
                let surface_dictionary: DictionaryFamily<Instances> =
                    bincode::deserialize_from(source).expect("Could not decode cdf");
                surface_dictionary
            }
        };
        ...
        Ok(::Format::Entropy {
            options: Options {
                ...
                split_streams,
                content_window_len,
                ..Options::new(spec, probabilities, known_values)
            },
        })
    }
}
